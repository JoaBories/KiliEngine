...
PlanetMeshComponent::PlanetMeshComponent(
	...
	const float pRotateSpeed, const float pSeaLevel, const float pReliefScale, const float pMinDiffuse) :
	...
	mRotateSpeed(pRotateSpeed), mSeaLevel(pSeaLevel), mReliefScale(pReliefScale), mMinDiffuse(pMinDiffuse)
{
	SetName("PlanetMesh");
...
void PlanetMeshComponent::DrawFirst(Material* pMat) // planet
{
	...
	pMat->SetFloat("uMinDiffuse", mMinDiffuse);
	
	pMat->SetInt("GroundTexture", 0);
	pMat->SetInt("SeaTexture", 1);
	
	glActiveTexture(GL_TEXTURE0);
	if (mGroundTexture) mGroundTexture->SetActive();
	
	glActiveTexture(GL_TEXTURE1);
	if (mSeaTexture) mSeaTexture->SetActive();
